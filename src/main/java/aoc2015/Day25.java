...
// 
// After putting you on hold for twenty minutes (your call is very important to them, it reminded you repeatedly), they finally find an engineer 
// that remembers how the code system works.
// 
// The codes are printed on an infinite sheet of paper, starting in the top-left corner. The codes are filled in by diagonals: starting with the 
// first row with an empty first box, the codes are filled in diagonally up and to the right. This process repeats until the infinite paper is covered. 
...
public class Day25 {
	public static final int INITIAL_CODE = 20151125;
	public static final int MULTIPLIER = 252533;
	public static final int DIVIDER = 33554393;

	public static final int ROW = 2981;
	public static final int COLUMN = 3075;

	public static void main(String[] args) {
		BigInteger code = BigInteger.valueOf(INITIAL_CODE);
		int repeats = (COLUMN > 1) ? ROW + COLUMN - 1 : ROW;

		for (int i = 2; i <= repeats; i++) {
			for (int y = i; y > 0; y--) {
				int x = i - y + 1;

				code = code.multiply(BigInteger.valueOf(MULTIPLIER)).remainder(BigInteger.valueOf(DIVIDER));
				if (y == ROW && x == COLUMN) {
